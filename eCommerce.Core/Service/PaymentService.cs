...
namespace eCommerce.Core.Service
{
    public class PaymentService : IPaymentService
    {
        ...
        public Task<PaymentDTO?> GetByIdAsync(Guid paymentId)
            => _repo.GetById(paymentId);

        public Task<IEnumerable<PaymentDTO>> GetByBookingAsync(Guid bookingId)
            => _repo.GetByBooking(bookingId);

        public async Task<PaymentDTO> CreateAsync(PaymentCreateDTO dto)
        {
            if (dto.Amount <= 0)
                throw new ArgumentOutOfRangeException(nameof(dto.Amount), "Amount must be > 0.");
            if (string.IsNullOrWhiteSpace(dto.Method))
                throw new ArgumentException("Method is required.", nameof(dto.Method));
        ...
        {
            // ID comes from the route param (paymentId). Do not read dto.PaymentId.
